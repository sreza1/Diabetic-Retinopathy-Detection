#trist to remove unnecessary black borders around the images,and
#"trim" the images so they take up the entirety of hte iamge.
...
def trim(im):
    # converts image to grayscale using cv2, then computers binary matrix of 
    # the pixels that are above a certain threshold, then takes out
    # the first rrow where a certain percentage of the pixels are above the threshold 
    # will be the first clip point. Same idea for col, max row, max col
...
def fast_image_resize(input_path_folder, output_path_folder, output_size=None):
    #uses multi processing to make it fast
    if not output_size:
